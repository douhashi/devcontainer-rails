...
RSpec.describe 'ContentPlayButtonController', type: :component do
  describe 'JavaScript functionality' do
    ...
    context 'when testing the controller logic' do
      it 'has the correct controller name and action mapping' do
        ...
        expect(File.exist?(Rails.root.join('app/frontend/controllers/content_play_button_controller.js'))).to be_truthy

        controller_content = File.read(Rails.root.join('app/frontend/controllers/content_play_button_controller.js'))
  ...
  describe 'Component Integration' do
    ...
    it 'component renders with correct data attributes' do
      component = ContentPlayButton::Component.new(content_record: content)
      ...
      expect(rendered_html).to have_selector(
        'button[data-controller="content-play-button"]'
      )
      expect(rendered_html).to have_selector(
        'button[data-action="click->content-play-button#playContent"]'
      )
      expect(rendered_html).to have_selector(
        'button[data-content-play-button-content-id-value="' + content.id.to_s + '"]'
      )
      expect(rendered_html).to have_selector(
        'button[data-content-play-button-theme-value="' + content.theme + '"]'
      )
    end
  end
end
